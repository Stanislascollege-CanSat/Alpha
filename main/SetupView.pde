...
public class SetupView extends ViewController {
  ...
  public NumberLineInputElement Select_baudRate;
  ...
  public SetupView(AppController a, float x, float y, float w, float h){
    ...
    this.baudRates = new ArrayList<Integer>();
    ...
    this.Select_baudRate = new NumberLineInputElement(this.appController, this.dim.x/2 + 10, this.dim.y/4 + 100, 200){
      public void enterEvent(){
        println(this.getNumber());
        this.reset();
    ...
    this.Button_continue = new ButtonElement(this.appController, this.dim.x/2 + 10, this.dim.y - 40, 100, "Continue"){
      public void clickEvent(){
        showMessageDialog(null, "What do you mean 'look at location 0x" + str(int(random(111111111, 999999999))) + "'?!\nIT'S EMPTY!", "Work in progress", ERROR_MESSAGE);
      }
    };
